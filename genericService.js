...
myServices.factory('myService', function ($http) {
    return {
        GetDataFromService: function (mockEnabled, searchText) {
            if (mockEnabled) {
                return $http.get('foo.json');
            }
            else {
                var url = 'http://localhost:52510/api/person?searchText=' + searchText + '&callback=JSON_CALLBACK';
                $http.jsonp(url)
